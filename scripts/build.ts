...
const saveToJson = async ({
  ...
  const dbData: {
    ...
    chapters:content.map((item,index) => ({...item,order:index+1})),
    app_name,
    app_icon,
    cover:splash_url,
    main_color,
...
const downloadAssets = async (content: IChapter[], userId: string): Promise<IChapter[]> => {
  ...
  const downloadPromises = content.map(async (item) => {
    ...
    const coverPath = path.join(assetsDir, item.cover.split("/").pop()!);
    await downloadFile(item.cover, coverPath, userId);

    // Process articles
    const newArticles: IArticle[] = await Promise.all(item.articles.map(async (article) => {
      // Download article icon
      const iconPath = path.join(assetsDir, article.icon.split("/").pop()!);
      await downloadFile(article.icon, iconPath, userId);

      // Process items
      const newItems: IContentItems[] = await Promise.all(article.items.map(async (item) => {
        if (item.type === "image") {
          const imagePath = path.join(assetsDir, item.content.split("/").pop()!);
          await downloadFile(item.content, imagePath, userId); // Corrected: Use item.content for the download

          return {
            type: "image",
            content: imagePath.split("/").slice(-2).join("/"),
            index: item.index,
          };
        }
    ...
    return {
      ...
      cover: coverPath.split("/").slice(-2).join("/"),
    };
  });
...
const main = async () => {
  ...
  const content: IChapter[] = metadata.content;
